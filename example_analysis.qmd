...
Numerous studies have shown that modern agriculture brings great damage to our ecosystem through accelerating climate change, which result in higher temperature increases and less rainfall.[@farmingresult]

The goal of this example analysis is to further explore the relationship between temperature and rainfall. Then zoom in to one city and study the distribution of rainfall for a given year. To answer the question above, I will do the following:

1\. make a line plot of the max and min temperature (y-axis) over time (x-axis) for each city.

2\. plots the estimates of mean rainfall and the 95% confidence intervals on the same plot. There should be a separate faceted plot for each city.
...
The two datasets I will be using come from [TidyTuesday](https://www.tidytuesday.com). Specifically, I will use the following data from January 2020.
...
If you want to find out what the column names represent, here is the data dictionary for the dataset: - <https://github.com/rfordatascience/tidytuesday/blob/master/data/2020/2020-01-07/readme.md#data-dictionary> [@datadictionary]
...
```{r question 2}
# plot 3 using geom_point and geom_errorbar and facet_warp
...
::: callout-tip
Using the wrangled data from research question 1.
:::
...
Functions used in data manipulation: unite(), mutate(), select(), inner_join(), glimpse(), filter(), group_by(), summarize().

Functions used in data visualization: geom_line(), geom_point(), geom_error_bar(), geom_histogram, facet_wrap()

# Findings

Canberra and Sydney has greatest variation in temperature over time, and they also have lowest amount of rainfall. Perth and Melbourne has least variation in temperature, and they also have greatest amount of rainfall. Thus, temperature and rainfall seems to be negatively correlated with each other.
